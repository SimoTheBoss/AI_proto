    ...
    while not finished:
        ...
        if len(task.yields) > 0:
            ...
            if flag == "preview":
                ...
                ) if image is not None else gr.update(), gr.update(), gr.update(
                    visible=False
                )
            if flag == "results":
    ...
    with gr.Row():
        with gr.Column(scale=2):
            with gr.Row():
                with gr.Column(scale=2):  # Il titolo prende più spazio
                    gr.Markdown(
                        "<h1 id='custom-title'>Welcome on ElypsiaArt.ai</h1>",
            ...
            with gr.Row():
                with gr.Row(
                    ...
                    with gr.Tabs(
                        selected=modules.config.default_selected_image_input_tab_id
                    ):
                        with gr.Tab(
                            label="Upscale or Variation", id="uov_tab"
                        ) as uov_tab:
                            with gr.Row():
                                with gr.Column():
                                ...
                                for image_count in range(
                                    ...
                                    with gr.Column():
                                        ip_image = grh.Image(
                                            ...
                                            show_label=False,
                                            height=300,
                                            value=modules.config.default_ip_images[
                                                image_count
                                            ],
                                        )
                                        ...
                                        with gr.Column(
                                            visible=modules.config.default_image_prompt_advanced_checkbox
                                        ) as ad_col:
                                            with gr.Row():
                                                ip_stop = gr.Slider(
                                                    label="Stop At",
                                                    ...
                                                    maximum=1.0,
                                                    step=0.001,
                                                    value=modules.config.default_ip_stop_ats[
                                                        image_count
                                                    ],
                                                )
                                                ...
                                                ip_weight = gr.Slider(
                                                    ...
                                                    maximum=2.0,
                                                    step=0.001,
                                                    value=modules.config.default_ip_weights[
                                                        image_count
                                                    ],
                                                )
                                            ...
                                            ip_type = gr.Radio(
                                                ...
                                                value=modules.config.default_ip_types[
                                                    image_count
                                                ],
                                                container=False,
                                            )
                        ...
                        with gr.Tab(
                            label="Inpaint or Outpaint", id="inpaint_tab"
                        ) as inpaint_tab:
                            with gr.Row():
                                with gr.Column():
                                ...
                                with gr.Column(
                                    visible=modules.config.default_inpaint_advanced_masking_checkbox
                                ) as inpaint_mask_generation_col:
                                    ...
                                    example_inpaint_mask_dino_prompt_text = gr.Dataset(
                                        ...
                                        visible=modules.config.default_inpaint_mask_model
                                        == "sam",
                                    )
                                    ...
                                    with gr.Accordion(
                                        "Advanced options", visible=False, open=False
                                    ) as inpaint_mask_advanced_options:
                                    ...
                                    def generate_mask(
                                        ...
                                        from extras.inpaint_mask import (
                        ...
                        with gr.Tab(
                            label="Describe", id="describe_tab"
                        ) as describe_tab:
                            with gr.Row():
                                with gr.Column():
                        ...
                        with gr.Tab(
                            label="Metadata", id="metadata_tab"
                        ) as metadata_tab:
                            with gr.Column():
                            ...
                            def trigger_metadata_preview(file):
                                (
                                    parameters,
                                    metadata_scheme,
                                ) = modules.meta_parser.read_info_from_image(file)
                            ...
                            with gr.Row():
                                with gr.Column():
                                    ...
                                    enhance_uov_prompt_type = gr.Radio(
                                        ...
                                        visible=modules.config.default_enhance_uov_processing_order
                                        == flags.enhancement_uov_after,
                                    )

                                    enhance_uov_processing_order.change(
                                        lambda x: gr.update(
                                            visible=x == flags.enhancement_uov_after
                                        ),
                        ...
                        for index in range(modules.config.default_enhance_tabs):
                            with gr.Tab(label=f"#{index + 1}") as enhance_tab_item:
                                ...
                                enhance_mask_dino_prompt_text = gr.Textbox(
                                    ...
                                    placeholder="Describe what you want to detect.",
                                    interactive=True,
                                    visible=modules.config.default_enhance_inpaint_mask_model
                                    == "sam",
                                )
        ...
        with gr.Column(
            ...
            with gr.Tab(label="Settings"):
                ...
                with gr.Accordion(
                    label="Aspect Ratios", open=False, elem_id="aspect_ratios_accordion"
                ...
                with gr.Column(
                    ...
                    with gr.Tab(label="Debug Tools"):
                        ...
                        vae_name = gr.Dropdown(
                            label="VAE",
                            choices=[modules.flags.default_vae]
        ...
        if not args_manager.args.disable_preset_selection:
            ...
            def preset_selection_change(preset, is_generating, inpaint_mode):
                preset_content = (
                    modules.config.try_get_preset_content(preset)
                    if preset != "initial"
                    else {}
                )
                ...
                (
                    preset_prepared["base_model"],
                    preset_prepared["checkpoint_downloads"],
                ...
                return modules.meta_parser.load_parameter_button_click(
                    json.dumps(preset_prepared), is_generating, inpaint_mode
                )
        ...
        for mode, disable_initial_latent, engine, strength, respective_field in [
            default_inpaint_ctrls
        ] + enhance_inpaint_update_ctrls:
            shared.gradio_root.load(
                inpaint_mode_change,
        ...
        def trigger_metadata_import(file, state_is_generating):
            ...
            return modules.meta_parser.load_parameter_button_click(
                parsed_parameters, state_is_generating, inpaint_mode
            )
